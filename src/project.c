...
/* create thread type for modbusrun_id*/
...
static int Update_Analog_Input_Read_Property(BACNET_READ_PROPERTY_DATA *rpdata){
	...
	/* get first list head for each instance and store into current_object*/
	...
	free(current_object);// free the current object
   }
  	not_pv:
   	/*unlock*/
...
/* Flush list to reduce memory usage*/
...
int main(int argv, char **argc){
	...
	bacnet_Device_Set_Object_Instance_Number(BACNET_INSTANCE_NO);
    	...
    	while (1) {
		pdu_len =
	    	bacnet_datalink_receive(&src, rx_buf, bacnet_MAX_MPDU,BACNET_SELECT_TIMEOUT_MS);
		if (pdu_len) {
